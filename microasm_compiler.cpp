...
std::string Compiler::resolveIncludePath(const std::string& includePath) {
    ...
    throw std::runtime_error("Include file not found: " + includePath + " (tried " + finalPathMas.string() + " and " + finalPathMasm.string() + ")");
}
...
void Compiler::parseFile(const std::string& filePath) {
    ...
    try {
        ...
        for (const std::string& line : lines) {
            parseLine(line); // Process each line
        }
...
void Compiler::parseLine(const std::string& line) {
    std::string trimmedLine = trim(line);
    if (trimmedLine.empty() || trimmedLine[0] == ';') return; // Skip comments and empty lines

    if (debugMode) std::cout << "[Debug][Compiler] Parsing line: " << trimmedLine << "\n";
...
ResolvedOperand Compiler::resolveOperand(const std::string& operand, Opcode contextOpcode) {
    ...
    if (operand[0] == '#') { // Label address (for jumps/calls)
        if (labelMap.count(operand)) {
         ...
         // Check for Register Address ($R<n> or $R<name>) *before* immediate check
         else if (operand.length() > 1 && operand[1] == 'R') { // Potential register address like $R1 or $RAX
             std::string regName = operand.substr(1); // Extract potential register name (e.g., R1, RAX)
             std::string upperRegName = regName;
             std::transform(upperRegName.begin(), upperRegName.end(), upperRegName.begin(), ::toupper);

             // Use the same regMap as regular register parsing
             ...
             if (regMap.count(upperRegName)) { // Check if it's a known register name
                 int regIndex = regMap.at(upperRegName);
                 if (regIndex == -1) throw std::runtime_error("Cannot directly use RIP as operand");
                 result.type = OperandType::REGISTER_AS_ADDRESS; // Use the new type
                 result.value = regIndex;
             } else {
                  // If not a named register, check if it's R<num> format
                  if (regName.length() > 1 && std::isdigit(regName[1])) {
                      try {
                          ...
                          if (regIndexNum >= 0 && regIndexNum <= 15) {
                              ...
                              throw std::runtime_error("Register index out of range (R0-R15) for $ operand: " + operand);
                          }
                      } catch (...) {
                          // Fall through to immediate parsing if stoi fails or if it wasn't R<num>
                          goto parse_dollar_immediate; // Use goto to avoid duplicating immediate parsing logic
                      }
                  } else {
                      // Not a known named register and not R<num>, try immediate
                      goto parse_dollar_immediate;
                  }
             }
         }
         else {
